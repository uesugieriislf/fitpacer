...
export default defineComponent({
  setup() {
    ...
    // === 选择器：从计划推断当天训练类型 ===
    const planDay = computed(() => planStore.plan.find((p) => p.date === dateStr.value) ?? null);
    ...
    return () => (
      <div class="record-view view">
        ...
        <div class="view-body">
          {/* 日期导航 */}
          <div class="date-nav">
            <button class="btn btn-ghost date-arrow" onClick={() => changeDate(-1)}>
              ‹
            </button>
            <div class="date-display">
              <span class="date-main">
                {dateObj.value.getMonth() + 1}/{dateObj.value.getDate()}
              </span>
            ...
            {!isToday(dateStr.value) && (
              <button class="btn btn-sm btn-outline today-jump" onClick={goToday}>
                回到今天
              </button>
            )}
          </div>
            ...
            {recordStore.todayRecords.map((rec) => (
              <div class="record-card card" key={rec.id}>
                <div class="rec-top">
                  ...
                  <button
                    class="btn btn-ghost rec-del"
                    onClick={() => recordStore.removeRecord(rec.id)}
                  >
                    ✕
                  </button>
                </div>
                <div class="rec-meta">
                  {isCardioRecord(rec.action) ? (
                    <>
                      ...
                      {rec.note?.startsWith("❤️") && (
                        <span class="rec-hr">
                          {rec.note.split("❤️")[1]?.split(" ·")[0]?.trim()} bpm
                        </span>
                      )}
                    </>
                  ) : (
                ...
                {rec.note?.startsWith("❤️") && rec.note.includes("·") && (
                  <div class="rec-note">{rec.note.split(" · ")[1]}</div>
                )}
              </div>
            ))}
          </div>
            ...
            <div class="modal-content" onClick={(e) => e.stopPropagation()}>
                ...
                {ALL_GROUPS.map((g) => (
                  <div class="rec-group" key={g}>
                    <div class="rec-group-head">
                      <span class="rec-group-icon">
                        {g === "custom" ? "✏️" : MUSCLE_GROUP_ICONS[g]}
                      </span>
                      <span class="rec-group-label">
                        {g === "custom" ? "自定义" : MUSCLE_GROUP_LABELS[g]}
                      </span>
                    </div>
                    ...
                    {g === "custom" ? (
                      <div class="rec-custom-area">
                        <input
                          v-model={customAction.value}
                          class="text-input"
                          placeholder="输入动作名称，回车添加..."
                      ...
                      <div class="rec-chip-grid">
                        {exerciseStore.getExercisesByGroup(g).map((ex) => (
                          <button
                            ...
                            onClick={() => {
                              formAction.value = ex.name;
                              customAction.value = "";
                            }}
                          >
                            {ex.name}
                            {"id" in ex && (ex as any).id && <span class="rec-chip-tag">自</span>}
                          </button>
                        ))}
                      </div>
                    )}
              ...
              {selectedIsCardio.value && (
                <>
                  <div class="form-field">
                    ...
                    <div class="stepper-row">
                      <button
                        ...
                        onClick={() => {
                          cardioDuration.value = Math.max(5, cardioDuration.value - 5);
                        }}
                      >
                        −
                      </button>
                      ...
                      <button
                        class="btn btn-ghost btn-sm"
                        onClick={() => {
                          cardioDuration.value = Math.min(180, cardioDuration.value + 5);
                        }}
                      >
                        +
                      </button>
                  ...
                  <div class="form-field">
                    <div class="label">
                      平均心率 <span class="label-optional">（可选）</span>
                    </div>
                    <div class="stepper-row">
                      <button
                        class="btn btn-ghost btn-sm"
                        onClick={() => {
                          cardioHeartRate.value =
                            (cardioHeartRate.value || 120) > 60
                              ? (cardioHeartRate.value || 120) - 5
                              : 60;
                        }}
                      >
                        −
                      ...
                      <span class="stepper-value">
                        {cardioHeartRate.value ? cardioHeartRate.value + " bpm" : "未设置"}
                      </span>
                      <button
                        class="btn btn-ghost btn-sm"
                        onClick={() => {
                          cardioHeartRate.value = Math.min(220, (cardioHeartRate.value || 120) + 5);
                        }}
                      >
                        +
                      </button>
              ...
              {!selectedIsCardio.value && (
                <>
                    ...
                    <div class="form-field">
                      ...
                      <div class="stepper">
                        <button
                          class="btn btn-sm"
                          onClick={() => {
                            formSets.value = Math.max(1, formSets.value - 1);
                          }}
                        >
                          −
                        </button>
                        ...
                        <button
                          class="btn btn-sm"
                          onClick={() => {
                            formSets.value = Math.min(10, formSets.value + 1);
                          }}
                        >
                          +
                        </button>
                    ...
                    <div class="form-field">
                      ...
                      <div class="stepper">
                        <button
                          class="btn btn-sm"
                          onClick={() => {
                            formReps.value = Math.max(1, formReps.value - 1);
                          }}
                        >
                          −
                        </button>
                        ...
                        <button
                          class="btn btn-sm"
                          onClick={() => {
                            formReps.value = Math.min(50, formReps.value + 1);
                          }}
                        >
                          +
                        </button>
                  ...
                  <div class="form-field">
                    <div class="label">
                      RPE {formRpe.value} — {rpeLabels[formRpe.value] ?? ""}
                    </div>
                    <input
                      type="range"
                      min="1"
                      max="10"
                      v-model={[formRpe.value, ["number"]]}
                      class="slider"
                    />
                    <div class="rpe-marks">
                      <span>极轻</span>
