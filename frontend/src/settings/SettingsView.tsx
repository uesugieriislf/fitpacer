      ...
      <div class="settings-view view">
        ...
        <div class="view-body">
            ...
            <div class="form-field">
              ...
              <div class="input-with-unit">
                <input type="number" value={goalStore.config.height}
                  onInput={(e: Event) => goalStore.updateConfig({ height: parseFloat((e.target as HTMLInputElement).value) || 170 })}
              ...
              <div class="form-field">
                ...
                <div class="input-with-unit">
                  <input type="number" value={goalStore.config.startWeight}
                    onInput={(e: Event) => goalStore.updateConfig({ startWeight: parseFloat((e.target as HTMLInputElement).value) || 80 })}
              ...
              <div class="form-field">
                ...
                <div class="input-with-unit">
                  <input type="number" value={goalStore.config.targetWeight}
                    onInput={(e: Event) => goalStore.updateConfig({ targetWeight: parseFloat((e.target as HTMLInputElement).value) || 65 })}
                    class="text-input" step="0.1" min="30" max="200" />
                  <span class="unit">kg</span>
                </div>
              </div>
            ...
            <div class="day-toggles">
              {allDays.map(day => (
                <button key={day}
                  ...
                  onClick={() => onToggleDay(day)}>周{weekdayLabels[day]}</button>
              ))}
            </div>
          </section>
              ...
              <div class="form-field">
                <div class="input-with-unit">
                  <input type="number" value={settingsStore.settings.cardioTargetMinutes}
                    onInput={(e: Event) => settingsStore.setCardioTarget(parseInt((e.target as HTMLInputElement).value) || 150)}
          ...
          <section class="section">
            ...
            <div class="time-row">
              <input type="time" value={settingsStore.settings.sleepReminderTime}
                onChange={(e: Event) => settingsStore.setSleepReminderTime((e.target as HTMLInputElement).value)} class="text-input time-input" />
            </div>
          </section>
          ...
          <section class="section">
            ...
            <div class="theme-row">
              <button class={['btn', themeResult.theme.value === 'light' ? 'btn-primary' : 'btn-outline']} onClick={themeResult.toggle}>
          ...
          <section class="section">
            <div class="section-header">
              ...
              <button class="btn btn-primary btn-sm" onClick={openAddLib}>+ 添加</button>
            </div>
                ...
                {exerciseStore.allCustom.map(ex => (
                  <div key={ex.id} class="ex-lib-item card">
                    ...
                    <div class="ex-lib-actions">
                      <button class="btn btn-ghost btn-xs" onClick={() => openEditLib(ex.id)}>编辑</button>
                      <button class="btn btn-ghost btn-xs ex-del" onClick={() => confirmRemove(ex.id)}>删除</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Teleport to="body">
            {showLibModal.value && (
              <div class="modal-overlay" onClick={(e: MouseEvent) => { if (e.target === e.currentTarget) showLibModal.value = false }}>
                <div class="modal-content">
                  <h3>{editingId.value ? '编辑动作' : '添加动作'}</h3>

                  <div class="form-field">
                    <div class="label">动作名称</div>
                    <input v-model={libFormName.value} class="text-input" placeholder="如：哑铃弯举" />
                  </div>
                  ...
                  <div class="form-field">
                    ...
                    <input v-model={libFormPrescription.value} class="text-input" placeholder="如：3×10" />
                  </div>

                  <div class="form-actions">
                    <button class="btn btn-ghost" onClick={() => showLibModal.value = false}>取消</button>
                    <button class="btn btn-primary" onClick={submitLibForm}>保存</button>
                  </div>
                </div>
              </div>
            )}
