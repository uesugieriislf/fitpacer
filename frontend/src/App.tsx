...
export default defineComponent({
  setup() {
    ...
    function switchTab(key: NavKey) {
      ...
      slideDir.value =
        tabOrder.value.indexOf(key) > tabOrder.value.indexOf(activeTab.value) ? "right" : "left";
      ...
      <div class="app-shell">
        <main class="main-content">
          <div
            ...
            class={[
              "tab-panel",
              slideDir.value === "right" ? "slide-in-right" : "slide-in-left",
            ].join(" ")}
          >
        ...
        <nav class="bottom-nav">
          {navItems.map((item) => (
            <button
              key={item.key}
              class={["nav-item", activeTab.value === item.key && "active"]
                .filter(Boolean)
                .join(" ")}
