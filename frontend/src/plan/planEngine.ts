...
/** 有氧训练动作定义 */
export const CARDIO_ACTIONS: string[] = [
  "慢跑",
  "跳绳",
  "骑行",
  "游泳",
...
export function getCardioExercises(isLong: boolean): ExerciseItem[] {
  const names = isLong ? ["慢跑（长有氧）"] : ["慢跑", "跳绳"];
  return names.map((name) => ({
    name,
    prescription: isLong ? "60-70分钟" : "35-40分钟",
    completed: false,
  }));
}
...
export function moveToSlot(plan: DayPlan[], fromDate: string, toDate: string): DayPlan[] {
  ...
  return plan.map((p) => {
    ...
    if (p.date === toDate) {
      // 目标日期继承原训练内容，标注来源
      const baseDetails =
        fromDay.type === "strength" ? getStrengthDetails() : fromDay.details || "";
      const exercises =
        fromDay.type === "strength"
          ? []
...
function realignStrength(plan: DayPlan[], missedDate: string): DayPlan[] {
  ...
  for (let si = 0; si < strengthIndices.length; si++) {
    const prevDate = si === 0 ? firstSlot : newPlan[strengthIndices[si - 1]].date;
