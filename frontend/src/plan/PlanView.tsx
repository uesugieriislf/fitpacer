...
export default defineComponent({
  setup() {
    ...
    const cardioPct = computed(() =>
      Math.min(100, Math.round((store.weekCardioMinutes / cardioTarget.value) * 100)),
    );
    ...
    const todayAction = computed<{
      ...
      if (tp.type === "cardio") {
        ...
        return {
          ...
          sub: done ? "已完成" : tp.details || "35-40 分钟",
        };
      }
    ...
    return () => (
      <div class="plan-view view">
        <div class="view-header">
          ...
          {store.hasPlan && (
            <div class="today-bar">
              <div class="today-bar-main">
                ...
                <span class="today-date-text">
                  {weekdayName.value} · {new Date().getMonth() + 1}月
                </span>
                <span
                  class="today-badge"
                  style={{
                    background: todayAction.value.color + "20",
                    color: todayAction.value.color,
                  }}
                >
          ...
          <div class="week-nav">
            <button class="btn btn-ghost week-arrow" onClick={() => store.goToWeek(-1)}>
              ‹
            </button>
        ...
        <div class="view-body">
              ...
              {weekInfo.value.days.map((day: DayPlan) => (
                <div
                  ...
                  class={[
                    ...
                    day.completed && "day-done",
                    day.missed && "day-skipped",
                  ]
                    .filter(Boolean)
                    .join(" ")}
                >
                  ...
                  <div class="day-top">
                    <div class="day-date-block">
                      ...
                      <span class="day-date-wd">
                        周{weekdayLabels[new Date(day.date + "T00:00:00").getDay()]}
                      </span>
                    </div>
                    ...
                    <span class={["badge", getBadgeClass(day.type)].join(" ")}>
                  ...
                  {day.type !== "rest" && (
                    <div class="day-detail-area" onClick={() => toggleExpand(day.date)}>
                      ...
                      <span
                        class={["expand-chevron", isExpanded(day.date) && "chevron-open"]
                          .filter(Boolean)
                          .join(" ")}
                      >
                        ▸
                    ...
                    <div class="exercise-list">
                      ...
                      {MUSCLE_GROUPS.map((mg) => (
                        <div
                          ...
                          class={[
                            "mg-summary-item",
                            mgCompletion(day)[mg].done > 0 && "mg-summary-done",
                          ]
                            .filter(Boolean)
                            .join(" ")}
                        >
                          ...
                          <span class="mg-summary-count">
                            {mgCompletion(day)[mg].done}/{mgCompletion(day)[mg].total}
                          </span>
                      ...
                      <div
                        class={[
                          "mg-summary-item",
                          (day.warmupDone || day.cooldownDone) && "mg-summary-done",
                        ]
                          .filter(Boolean)
                          .join(" ")}
                      >
                        ...
                        <span class="mg-summary-badge">
                          {day.warmupDone && day.cooldownDone
                            ? "✅"
                            : day.warmupDone || day.cooldownDone
                              ? "⚠️"
                              : "○"}
                  ...
                  {day.type !== "rest" && !day.completed && !day.missed && (
                    <div class="day-foot">
                      <button
                        ...
                        onClick={() =>
                          day.type === "cardio"
                            ? openCardioModal(day.date)
                            : openStrengthModal(day.date)
                        }
                      >
                        {day.type === "cardio" ? "完成" : "完成"}
                      </button>
                      <button class="btn btn-ghost btn-sm" onClick={() => store.skipDay(day.date)}>
                        跳过
                      </button>
                    </div>
                  )}
                  {day.completed && (
                    <div class="day-foot">
                      <span class="done-badge">
                        ✅ 已完成
                        {day.type === "cardio" && day.cardioRecord && (
                          <span> · {day.cardioRecord.durationMinutes}分钟</span>
                        )}
                        {day.type === "strength" && <span> · {completionPct(day)}%</span>}
                      </span>
                      <button
                        class="btn btn-ghost btn-sm"
                        onClick={() => store.markCompleted(day.date)}
                      >
                        撤销
                      </button>
                    </div>
                  ...
                  {day.missed && (
                    <div class="day-foot">
                      ...
                      <button class="btn btn-ghost btn-sm" onClick={() => store.undoSkip(day.date)}>
                        撤回
                      </button>
                    </div>
                  )}
          ...
          {!store.hasPlan && (
            <div class="empty-state">
              ...
              <p>还没有训练计划</p>
              <button class="btn btn-primary" onClick={() => store.initPlan()}>
                生成计划
              </button>
            </div>
          )}
          ...
          <Teleport to="body">
            {store.showAdjustModal && (
              <div
                ...
                onClick={(e) => {
                  if (e.target === e.currentTarget) store.cancelAdjust();
                }}
              >
          ...
          <Teleport to="body">
            {showDatePicker.value && (
              <div
                class="modal-overlay pv-modal"
                onClick={(e) => {
                  if (e.target === e.currentTarget) showDatePicker.value = false;
                }}
              >
                <div class="modal-content">
                  <h3>选择推迟日期</h3>
                  ...
                  <div class="slot-list">
                    {datePickerSlots.value.map((slot) => (
                      <div
                        ...
                        onClick={() => applyDatePick(slot.date)}
                      >
          ...
          <CardioModal
            ...
            onSubmit={(payload: any) => {
              store.saveCardioRecord(
                payload.date,
                payload.record,
                payload.warmupDone,
                payload.cooldownDone,
              );
            }}
          />
