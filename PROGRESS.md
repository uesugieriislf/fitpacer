...
| `RecordView.vue` | ✅ 完成 | 日期导航、预设动作库（按计划自动过滤力量/有氧）、自定义动作、组数/次数/RPE 滑块、备注；录入改为底部弹窗（modal-overlay），点击遮罩可关闭 |
...
| `DashboardView.vue` | ✅ 完成 | 周报卡片、🎯 目标进度卡片、BMI 显示、体重/腰围趋势折线图、身体数据录入弹窗（含睡眠时长）、历史列表、训练日历、训练统计 |
...
| 文件 | 状态 | 说明 |
|------|------|------|
| `calendarEngine.ts` | ✅ 完成 | 纯函数：getMonthGrid 生成 42 格月历网格 + getDayStatus 日期状态判断 |
| `calendarEngine.test.ts` | ✅ 完成 | 13 个测试用例，覆盖月历网格、状态推导、边界条件 |
| `useCalendar.ts` | ✅ 完成 | Composable：year/month 响应式状态 + 月份导航 |
| `CalendarPanel.vue` | ✅ 完成 | 月历组件（月份导航 + 星期行 + 日期网格 + 状态圆点 + 图例）；错过改为空心圆环 |

### stats/ — 训练统计 ✅ 新增

| 文件 | 状态 | 说明 |
|------|------|------|
| `statsEngine.ts` | ✅ 完成 | 纯函数：按时间段（周/月/季度/年）聚合 TrainingRecord，返回按动作统计 + 排序 |
| `statsEngine.test.ts` | ✅ 完成 | 8 个测试用例，覆盖时间段聚合、动作排名、空数据 |
| `StatsPanel.vue` | ✅ 完成 | 统计面板：时间粒度切换（周/月/季度/年）、概要卡片（训练天/总组数/总次数）、动作排名列表+迷你柱状条 |

### plan/ — 变更

| 文件 | 说明 |
|------|------|
| `usePlan.ts` | ✅ 新增自动记录：完成力量训练时每个动作自动写入 TrainingRecord；完成有氧时自动写入一条记录；撤销完成时清除自动记录 |
| `PlanView.vue` | ✅ 力量弹窗集成自定义动作库：推荐 chip 同时展示内置+自定义动作，自定义带"自"标记；添加自定义动作自动保存到库 |

### exercise/ — 动作库管理 ✅ 新增

| 文件 | 状态 | 说明 |
|------|------|------|
| `useExercise.ts` | ✅ 完成 | Pinia store：自定义动作 CRUD + 与内置库合并的 getter |
| `storage.ts` | ✅ 完成 | CustomExercise 类型（name/muscleGroup/prescription）+ localStorage 读写 |

### settings/ — 变更

| 文件 | 说明 |
|------|------|
| `SettingsView.vue` | ✅ 新增"动作库管理"区块：自定义动作列表展示、添加/编辑/删除弹窗 |
...
| 2026-05-06 | RecordView 录入表单由内联 panel 改为底部弹窗（modal-overlay + modal-content），点击遮罩可关闭 | #3 |
| 2026-05-06 | 新增 stats/ 模块：训练统计功能，按周/月/季度/年聚合训练记录，显示训练天数、组数/次数和按动作排名；错过状态改为空心圆环区分于力量橙色 | #3 |
| 2026-05-06 | 统一数据模型：计划完成时自动写入 TrainingRecord（力量每个动作一条，有氧一条），撤销时自动清除，记录成为唯一数据源 | #3 |
| 2026-05-06 | 力量完成弹窗升级：每个动作展开实际组数/次数/RPE 输入，优先使用用户输入值创建 TrainingRecord，而非 prescription 默认值 | #3 |
| 2026-05-06 | 力量训练按部位改造：从 4 个固定动作改为胸/肩背/腿 3 部位自由选择；新增 shared/exercises.ts 动作数据库（15个动作）；弹窗中可点击推荐 chip 添加、自定义输入、删除已选动作 | #3 |

## 已知问题 / 待办

- 睡眠提醒设置了时间，但未实现到点页面提示
- 训练偏好当前固定"力量+有氧混合"，UI 层面可扩展其他模式
- 身体数据旧记录的时间戳仍为 UTC 格式，已有记录的 createdAt 显示可能仍偏差 8 小时（新录入已修复）
## 变更记录
...
| 2026-05-07 | 修复身体数据录入时间戳时区 bug（toISOString UTC→本地时间） | #4 |
| 2026-05-07 | 新增 exercise/ 模块：自定义动作库管理。自定义动作持久化保存，力量弹窗中可复用，Settings 页面支持添加/编辑/删除 | #4 |
